//
//  Payload_Diagnostic_Message_NAck.swift
...
public class Payload_Diagnostic_Message_Nack:PayloadObject
{
    public func initialize_PayloadItems()
    {
        Payload = [
            ...
            TAG.NACK: Payload_Item_Type(_Position: 4,_Length: 1),
    ...
    {
        if((Payload[TAG.NACK]?.RawData) != nil)
        {
            return ((Payload[TAG.NACK]?.RawData[0])!)
        }
        else
        {
    ...
    public func GetPreviousDiagnosticMessage() -> [UInt8]!
    {
