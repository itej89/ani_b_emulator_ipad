//
//  VehicleIdentificationWithEIDDiagState.swift
...
public class VehicleIdentificationWithEIDDiagState:IDiagStateForSendRecieve
{
...
//        UDPListen.Instance.StartListening(remoteEndPoint: DOIPSession.Instance.RemoteEndPoint)
    ...
    private func FormatRequest() -> (Int, [UInt8]?)
    {
        ...
        let request:DOIPRequestObject = formatRequest.FormHeaderForDoIPFrame(payloadType: Request_Payload_Types.CODE.PLD_VEH_IDEN_REQ_EID)
        let payload:Payload_Vehicle_Request_EID = Payload_Vehicle_Request_EID()
        payload.SetEID(EID: DOIPParameters.Instance.EID)
