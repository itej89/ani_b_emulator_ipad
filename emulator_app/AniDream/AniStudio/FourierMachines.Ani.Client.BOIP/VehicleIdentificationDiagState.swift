//
//  VehicleIdentificationDiagState.swift
...
public class VehicleIdentificationDiagState:IDiagStateForSendRecieve
{
    ...
    private func FormatRequest() -> (Int, [UInt8]?)
    {
        ...
        let request:DOIPRequestObject = formatRequest.FormHeaderForDoIPFrame(payloadType: Request_Payload_Types.CODE.PLD_VEH_IDEN_REQ)
    ...
    {
        UDPBroadcast.Instance.UDPBroadcastPacket(packet: data)
        if(UDPBroadcast.Instance.ValidationErrors.Messages.count > 0)
        {
            return DIAGNOSTIC_STATUS.CODE.INTERNAL_ERROR.rawValue
        }
        else
