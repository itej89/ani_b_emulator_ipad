...
open class FDWaveformView: UIView {
    ...
    /*@IBInspectable*/ open var doesAllowScroll = true
     ...
     @IBInspectable open var TapPercent:CGFloat = 0.0
...
//        addGestureRecognizer(panRecognizer)
    ...
    func isWaveformRenderOperationDirty(_ renderOperation: FDWaveformRenderOperation?) -> Bool? {
        ...
        let verticalOverdrawRequested = Double(renderOperation.imageSize.height / frame.width)
        if !verticalOverdrawAllowed.contains(verticalOverdrawRequested) {
            return true
        }
        let horizontalOverdrawRequested = Double(renderOperation.imageSize.height / frame.width)
    ...
    override open func layoutSubviews() {
        ...
        let childFrame = CGRect(x: frame.height * scaleW * -scaleX,
                                y: 0,
                                width: frame.height * scaleW,
                                height: frame.width)
        imageView.frame = childFrame
        if let highlightedSamples = highlightedSamples, highlightedSamples.overlaps(zoomSamples) {
            clipping.frame = CGRect(x: frame.height * highlightClipScaleL,
                                    y: 0,
                                    width: frame.height * (highlightClipScaleR - highlightClipScaleL),
                                    height: frame.width)
            if 0 < clipping.frame.minX {
                highlightedImage.frame = childFrame.offsetBy(dx: frame.height * scaleW * -highlightScaleX, dy: 0)
            } else {
    ...
    func renderWaveform() {
        ...
        let widthInPixels = floor(frame.height * CGFloat(horizontalOverdrawTarget))
        let heightInPixels = frame.width * CGFloat(horizontalOverdrawTarget)
...
//            highlightedSamples = 0 ..< Int((CGFloat(zoomSamples.startIndex) + rangeSamples * recognizer.location(in: self).x / bounds.width))
            ...
            TapPercent = (TapLocation / Width) * 100
            delegate?.waveformDidEndScrubbing?(self)
        }
    }
}
